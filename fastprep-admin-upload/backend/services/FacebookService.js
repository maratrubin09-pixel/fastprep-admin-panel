...
class FacebookService {
      ...
      switch (type) {
        ...
        case 'quick_reply':
          payload = {
            ...
            message: {
              text: message,
              quick_replies: messageData.quickReplies || []
            }
          };
          break;
    ...
    try {
      ...
      if (!customer) {
        ...
        customer = await Customer.create({
          firstName: userProfile.first_name || 'Facebook',
          lastName: userProfile.last_name || 'User',
          phone: senderId,
          source: 'facebook',
          status: 'new',
          notes: `Facebook ID: ${senderId}`
        });
      }
  ...
  async processPostback(event) {
    try {
      ...
      console.log('Facebook postback:', { senderId, postback });
  ...
  async processDelivery(event) {
    try {
      ...
      console.log('Facebook delivery:', { senderId, delivery });
      
      return { status: 'processed' };
    } catch (error) {
      console.error('Error processing Facebook delivery:', error);
      throw error;
    }
  }
  ...
  async processRead(event) {
    try {
      ...
      console.log('Facebook read:', { senderId, read });
      
      return { status: 'processed' };
    } catch (error) {
      console.error('Error processing Facebook read:', error);
      throw error;
    }
  }
